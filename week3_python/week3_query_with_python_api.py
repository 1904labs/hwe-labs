...
# print(reviews.select('*').show(5, truncate=False))
...
# reviews.select("product_category").show(50)
...
# reviews.select(reviews.product_title, reviews.helpful_votes).orderBy(
#     reviews.helpful_votes.cast("Integer").desc()).show(1)
...
# reviewsInt = reviews.select("total_votes", "star_rating", "helpful_votes").withColumn("total_votes", col("total_votes").cast('integer')).withColumn("star_rating", col(
#     "star_rating").cast('integer')).withColumn("helpful_votes", col("helpful_votes").cast('integer'))
# reviewsInt.printSchema()

# Question 8: Find the date with the most purchases.
# Print the date and total count of the date with the most purchases

# reviews.groupBy(reviews["purchase_date"]).count().sort(desc("count")).show(1)
...
# reviewsWithStamp = reviews.withColumn(
...
# reviewsWithStamp.write.mode("overwrite").parquet(
#     "s3a://hwe-fall-2023/tseserman/bronze/reviews_static")
